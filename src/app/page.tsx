...
export default function PortfolioPage() {
  ...
  // Sort: red overall first, then yellow, then green/unscored
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-xl font-semibold text-white">Deal &amp; Pricing Governance</h1>
          ...
          <table className="w-full min-w-[700px] text-sm">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-900/60">
                ...
                <th className="text-center px-4 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide" title="Pricing Health — based on deal take rate and band compliance">
                  Pricing
                </th>
                <th className="text-center px-4 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide" title="Collection Health — based on AR days and overdue ratio">
                  Collection
                </th>
                <th className="text-center px-4 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide" title="Capacity Health — based on deal volume and strategic flag">
                  Capacity
                </th>
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      {shipper.strategic && (
                        <span className="text-xs bg-blue-950 text-blue-400 border border-blue-900 px-1.5 py-0.5 rounded">
                          Strategic
                        </span>
                      )}
                    </div>
                  ...
                  <td className="px-4 py-3 text-center">
                    <TrafficLight status={health.pricing} />
                  </td>
                  <td className="px-4 py-3 text-center">
                    <TrafficLight status={health.collection} />
                  </td>
                  <td className="px-4 py-3 text-center">
                    <TrafficLight status={health.capacity} />
                  </td>
                  <td className="px-4 py-3 text-center">
                    {flagged > 0 ? (
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link
                      ...
                      className="text-xs text-slate-500 hover:text-white px-2 py-1 rounded hover:bg-slate-800 transition-colors"
                    >
                      View →
                    </Link>
                  </td>
