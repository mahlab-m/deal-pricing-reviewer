  ...
  for (const result of shipperResults) {
    ...
    if (result.breaches.some((b) => b.rule === "data_quality")) {
      entries.push({
        ...
        priority: "P1",
        status: "not started",
        rationale: `${result.shipper_id} has null metrics — cannot score pricing or collection health. Data ops must populate take_rate_pct, ar_days, overdue_ratio before commercial review.`,
        created_at: now,
      });
      continue;
    }
